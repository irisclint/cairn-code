...
[GitHub security advisory](https://github.com/irisclint/cairn-code/security/advisories/new)
...
While cairn-code is in alpha, only the latest release receives fixes.

## How cairn-code protects you
...
**The custom protocol is scoped.** `cairn://` resolves only inside the opened
...
cairn-code sends nothing until it is switched on. The only other network request is
...
cairn-code loads no third party code at all.
